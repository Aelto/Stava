...
async function startInterval({gameServer, summonerName, apiKey, pathToLeagueFolder}) {
	...
	// stores the last spectated game ID during the session.
	// Pressing the "cancel" button and starting another interval
	// will reset it.
	let lastGameId = 0
	...
	const intervalFunction = async function () {
		...
		if (gameData.gameId === lastGameId)
			return console.log(`already notified user for game with id [${gameData.gameId}], skipping it.`)
